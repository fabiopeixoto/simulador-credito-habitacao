...
const SEED_BANKS = [
  ...
  { code: "CTT", name: "Banco CTT", color: "#e30613", refs: ["3m", "6m", "12m"], jOk: true, carenciaMax: 0, tipos: ["variável", "mista"], promos: ["Spread CH 0,70–2,60% (precário §18.1)", "Pack domiciliação + seguros (spread reduzido)", "CH Sinal Jovem: TAN fixa 5%"], prod: "Seg. Vida + Multirriscos + Dom. CTT", jProd: "Mista 2a ilustr. 3,65% fixo; Sinal Jovem 5% fixo" },
  ...
  { code: "MNTPO", name: "Banco Montepio", color: "#7a0f2e", refs: ["3m", "6m", "12m"], jOk: true, carenciaMax: 24, tipos: ["variável", "mista"], promos: ["CH §18.1: Euribor 3/6/12m + spread 0,70–2,30% (mai.2026)", "Cashback 1,5%"], prod: "Conta Ordenado + Cartão ≥500€/sem + Seg. Vida + Multirriscos Lusitania", jProd: "100% garantia Estado; cashback 1,5%" },
...
const SEED_SPREADS = {
  ...
  CTT: { sCom: 0.70, sSem: 2.60, mCom: 3.65, mSem: 5.35, fCom: 4.70, fSem: 4.70, jsCom: 0.70, jsSem: 2.60, promoPeriodo: 0, promoSpread: null, dossier: 280, avaliacao: 230, contaMes: 1.73, contaNota: "20€/ano conta + IS 4% (exemplos TAEG precário mai.2026)", capMin: 25000, capMax: 1000000, vRef: 15.71, mAno: 170, insV: "CTT Seguros", insM: "CTT Seguros", minutas: 160, jovemIsenta: false },
  ...
  SANTR: { sCom: 0.80, sSem: 1.90, mCom: 3.25, mSem: 4.35, fCom: 4.40, fSem: 4.40, jsCom: 0.80, jsSem: 1.90, promoPeriodo: 36, promoSpread: 0, dossier: 725, avaliacao: 230, contaMes: 2.90, contaNota: "Conta (estimativa; FINE CH exclui manutenção mensal da TAEG)", capMin: 30000, capMax: 3000000, vRef: 22.55, mAno: 246, insV: "Santander Seguros", insM: "Santander Seguros", minutas: 0, jovemIsenta: false },
...
module.exports = async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const maxFetchedAt = sqliteDb.prepare("SELECT MAX(fetched_at) AS mx FROM spreads").get()?.mx || 0;
    const etag = `"banks-${maxFetchedAt}"`;
