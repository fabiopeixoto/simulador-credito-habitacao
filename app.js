...
function App(props){
  ...
  // Chips-resumo: mostra só os principais; «+ ver mais» expande os restantes
...
React.createElement("button", {onClick:handleSave,"aria-label":"Guardar simulação",title:"Guardar simulação", style:{padding:"7px 10px",background:saved?"rgba(74,222,128,0.12)":"rgba(0,0,0,0.05)",border:"1px solid "+(saved?"rgba(74,222,128,0.5)":"rgba(0,0,0,0.09)"),borderRadius:7,color:saved?G:"#374151",fontSize:14,cursor:"pointer",transition:"all 0.2s"}}, saved?"✓":"💾"),
            ...
            ].filter(ch=>chipsExpand||["Capital","Prazo","Melhor banco","TOTAL (c/ tudo)","TAEG"].includes(ch.l)).map(({l,v,c="#111827"})=>(
            ...
            )), React.createElement("button", {onClick:()=>setChipsExpand(v=>!v),"aria-expanded":chipsExpand,style: {background:"rgba(37,99,235,0.08)",border:"1px solid rgba(37,99,235,0.3)",borderRadius:7,padding:"3px 9px",color:Au,fontSize:11,fontFamily:"sans-serif",cursor:"pointer",fontWeight:600}}, chipsExpand?"− ver menos":"+ ver mais"))), showHist&&histSaved.length>0&&React.createElement("div", {style:{position:"fixed",inset:0,zIndex:9999,background:"rgba(0,0,0,0.5)",display:"flex",alignItems:"center",justifyContent:"center"},onClick:()=>setShowHist(false)},
