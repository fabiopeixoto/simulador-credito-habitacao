...
React.createElement("button", {onClick:handleSave,"aria-label":"Guardar simulação",title:"Guardar simulação", style:{padding:"7px 10px",background:saved?"rgba(74,222,128,0.12)":"rgba(0,0,0,0.05)",border:"1px solid "+(saved?"rgba(74,222,128,0.5)":"rgba(0,0,0,0.09)"),borderRadius:7,color:saved?G:"#374151",fontSize:14,cursor:"pointer",transition:"all 0.2s"}}, saved?"✓":"💾"),
          ...
          React.createElement("div", {style: {marginBottom:10}}, React.createElement("div", {style: {display:"flex",flexWrap:"wrap",borderRadius:9,overflow:"hidden",border:"1px solid rgba(0,0,0,0.07)",background:IS_MOBILE?"rgba(0,0,0,0.06)":"rgba(255,255,255,1)",rowGap:IS_MOBILE?1:0}}, NAV.map(({id,icon,label})=>(
            React.createElement("button", {key: id, onClick: nav===id?undefined:()=>setNav(id), style: {flex:IS_MOBILE?"1 1 33%":1,padding:IS_MOBILE?"7px 4px":"9px",border:"none",background:nav===id?"rgba(37,99,235,0.08)":"rgba(255,255,255,1)",borderBottom:"2px solid "+(nav===id?Au:"transparent"),color:nav===id?Au:"#374151",fontSize:IS_MOBILE?12:13,fontFamily:"sans-serif",cursor:nav===id?"default":"pointer",fontWeight:600,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}, icon+" "+label)
          ))), nav==="comp"&&(
