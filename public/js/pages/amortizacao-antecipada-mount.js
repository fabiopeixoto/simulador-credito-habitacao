;(function(){
  ...
  if(!React||!ReactDOM||!window._SIM||!window.AmortizacaoAntPage){
  ...
  function Root(props){
    ...
    var _sg=useState(false);var showGlossario=_sg[0];var setShowGlossario=_sg[1];
    var _sp=useState(false);var showProcesso=_sp[0];var setShowProcesso=_sp[1];
    var _scom=useState([]);var comments=_scom[0];var setComments=_scom[1];
    ...
    return h(React.Fragment,null,
      ...
      showGlossario&&GlossarioModal&&h(GlossarioModal,{onClose:function(){setShowGlossario(false);}}),
      showProcesso&&ProcessoModal&&h(ProcessoModal,{onClose:function(){setShowProcesso(false);}})
    );
  }
  ...
  fetch('/api/banks')
    ...
    .then(function(raw){
      ...
      ReactDOM.createRoot(el).render(h(Root,{initialEUR:initialEUR}));
    })
    .catch(function(){
      ReactDOM.createRoot(el).render(h(Root,{initialEUR:null}));
    });
})();
